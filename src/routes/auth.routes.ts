...
import { authFloodLimiter, forgotPasswordLimiter, loginLimiter, registerLimiter } from '../middleware/rateLimit.ts';
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    // Self-service sign-up is switched off for now.
    //
    // The Login page already hides the "Create Account" form (see
    // src/lib/features.ts), but hiding a form is not the same as closing the
    // endpoint: the route stayed publicly callable, so anyone could still
    // POST here and create student accounts directly. Gating it server-side
    // makes the off switch real. Administrators create accounts through the
    // admin-only POST /api/admin/students instead, so nothing legitimate
    // depends on this staying open. Flip ALLOW_SELF_REGISTRATION to re-enable.
    if (!env.allowSelfRegistration) {
      throw new ForbiddenError(
        'Self-service registration is currently disabled. Please contact your teacher for an account.'
      );
    }
